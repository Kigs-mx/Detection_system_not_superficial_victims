...
scale = cv2.imread('/home/kigs/Pictures/img_thermal_tesis/color_palette/palette_d.png')
...
def color_presentation (cv_image):
  ...
  cv2.imshow("Color_thermal_image_d", output)
  key = cv2.waitKey(1)

  if key == ord("s"):
    i=i+1
    cv2.imwrite('/home/kigs/Pictures/img_thermal_tesis/thermal_image_color_d_%i.bmp'%i, output)
...
def main(args):
  ...
  i = 0
  rospy.init_node('color_monitor_d', anonymous=False)
