...
scale = cv2.imread('/home/kigs/Pictures/img_thermal_tesis/color_palette/palette_b.png')
...
def color_presentation (cv_image):
  ...
  lee_val=[]                          
  readFile=open("/home/kigs/catkin_ws/src/system_nsv_detector/find_victims/scripts/palette_b.txt","r")
  ...
  cv2.imshow("Color_thermal_image_b", output)
  key = cv2.waitKey(1)

  if key == ord("s"):
    i=i+1
    cv2.imwrite('/home/kigs/Pictures/img_thermal_tesis/thermal_image_color_b_%i.bmp'%i, output)
...
def main(args):
  ...
  i = 0
  rospy.init_node('color_monitor_b', anonymous=False)
