...
scale = cv2.imread('/home/kigs/Pictures/img_thermal_tesis/color_palette/palette_e.png')
...
def color_presentation (cv_image):
  ...
  lee_val=[]                          
  readFile=open("/home/kigs/catkin_ws/src/system_nsv_detector/find_victims/scripts/palette_e.txt","r")
  ...
  #print "scale"
  #print overlay.shape
  output = cv2.addWeighted(overlay, 1, output, 1, 0)
  
  #cv_image = imutils.resize(cv_image, width = 640)
  cv2.imshow("Color_thermal_image_e", output)
  key = cv2.waitKey(1)

  if key == ord("s"):
    i=i+1
    cv2.imwrite('/home/kigs/Pictures/img_thermal_tesis/thermal_image_color_e_%i.bmp'%i, output)
...
def main(args):
  ...
  rospy.init_node('color_monitor_e', anonymous=False)
